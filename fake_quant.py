...
def quantize_bagel(
    ...
    mode="gen" # 新增模式选择: "und" 或 "gen"
):
    """
    针对 Bagel (Qwen2-NaViT) 的专用量化函数。
    mode: 
      - 'und': 只量化文本理解相关的层 (Standard Attention, MLP)。
      - 'gen': 量化所有层，包括图像生成层 (_moe_gen)。
    """
    from modeling.bagel.qwen2_navit import (
        Qwen2DecoderLayer,
        Qwen2MoEDecoderLayer,
        Qwen2MoTDecoderLayer, # 包含 PackedAttentionMoT
        PackedAttention,
        PackedAttentionMoT,
        Qwen2MLP
    )

    print(f"Applying Bagel Quantization (Mode: {mode})...")

    # 获取模型骨干 (适配 Bagel 的封装结构)
    # 结构通常是: Bagel -> language_model -> model -> layers
    ...
    for i, layer in enumerate(layers):
        ...
        attn.q_proj = W8A8Linear.from_float(
            attn.q_proj, weight_quant=weight_quant, act_quant=act_quant, quantize_output=quantize_bmm_input
        )
        attn.k_proj = W8A8Linear.from_float(
            attn.k_proj, weight_quant=weight_quant, act_quant=act_quant, quantize_output=quantize_bmm_input
        )
        attn.v_proj = W8A8Linear.from_float(
            attn.v_proj, weight_quant=weight_quant, act_quant=act_quant, quantize_output=quantize_bmm_input
        )
        attn.o_proj = W8A8Linear.from_float(
            attn.o_proj, weight_quant=weight_quant, act_quant=act_quant
        )

        if mode == "gen" and hasattr(attn, "q_proj_moe_gen"):

            attn.q_proj_moe_gen = W8A8Linear.from_float(
                attn.q_proj_moe_gen, weight_quant=weight_quant, act_quant=act_quant, quantize_output=quantize_bmm_input
            )
            attn.k_proj_moe_gen = W8A8Linear.from_float(
                attn.k_proj_moe_gen, weight_quant=weight_quant, act_quant=act_quant, quantize_output=quantize_bmm_input
            )
            attn.v_proj_moe_gen = W8A8Linear.from_float(
                attn.v_proj_moe_gen, weight_quant=weight_quant, act_quant=act_quant, quantize_output=quantize_bmm_input
            )
            attn.o_proj_moe_gen = W8A8Linear.from_float(
                attn.o_proj_moe_gen, weight_quant=weight_quant, act_quant=act_quant
            )

        #MLP
        layer.mlp.gate_proj = W8A8Linear.from_float(
            layer.mlp.gate_proj, weight_quant=weight_quant, act_quant=act_quant
        )
        layer.mlp.up_proj = W8A8Linear.from_float(
            layer.mlp.up_proj, weight_quant=weight_quant, act_quant=act_quant
        )
        layer.mlp.down_proj = W8A8Linear.from_float(
            layer.mlp.down_proj, weight_quant=weight_quant, act_quant=act_quant
        )

        if mode == "gen" and hasattr(layer, "mlp_moe_gen"):

            layer.mlp_moe_gen.gate_proj = W8A8Linear.from_float(
                layer.mlp_moe_gen.gate_proj, weight_quant=weight_quant, act_quant=act_quant
            )
            layer.mlp_moe_gen.up_proj = W8A8Linear.from_float(
                layer.mlp_moe_gen.up_proj, weight_quant=weight_quant, act_quant=act_quant
            )
            layer.mlp_moe_gen.down_proj = W8A8Linear.from_float(
                layer.mlp_moe_gen.down_proj, weight_quant=weight_quant, act_quant=act_quant
            )
